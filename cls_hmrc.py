...
class HMRC:
    ...
    def __get_breakdown(self, category_like):
        ...
        query = self.transactions.query_builder().select('Date', 'Key', 'Description', 'Note', 'Nett', 'Category').where(f'"Tax year"="{tax_year}" AND "Category" LIKE "{category_like}%"').order('Date').build()
        self.l.debug(query)
        ...
        for row in rows:
            breakdown.append(f'{row[0]} | {row[1]} | {row[2][:max_description_width]} | {row[3]} | {row[4]:>10.2f} | {row[5][:max_category_width]}')
    ...
    def are_there_digest_transactions(self, digest_type) -> bool:
        ...
        tax_year = self.tax_year
        query = self.transactions.query_builder().select_raw('COUNT(DISTINCT Category)').where(f'"Tax year"="{tax_year}"' + f' AND "Category" LIKE "HMRC {person_code}{digest_category_like}%"').build()
        ...
        if amount <= available_allowance:
            ...
            if amount <= basic_rate_limit:
                ...
                if amount <= higher_rate_limit:
                    tax = amount * basic_tax_rate + (amount - basic_rate_limit) * higher_tax_rate
                else:
                    tax = amount * basic_tax_rate + (higher_rate_limit - basic_rate_limit) * higher_tax_rate + (amount - higher_rate_limit) * additional_tax_rate
        self.l.debug(f'tax: {tax}')
        return (round(tax, 2), available_allowance)
    ...
    def did_none_of_these_apply__business_1_page_1__(self):
        ...
        conditions = [self.are_you_a_foster_carer(), self.do_you_wish_to_make_an_adjustment_to_your_profits(), self.are_you_a_farmer(), self.were_any_results_already_declared_on_a_previous_return(), self.is_the_basis_period_different_to_the_accounting_period(), self.is_your_business_carried_on_abroad(), self.are_you_claiming__overlap_relief_(), self.should_you_pay_voluntary_class_2_nics__turnover___ta(), self.are_you_claiming_back_cis_tax_already_paid(), self.are_you_claiming_relief_for_a_loss()]
        self.l.debug(conditions)
        return uf.all_conditions_are_false(conditions)

    def did_none_of_these_apply__class_4_nics_(self):
        conditions = [self.were_you_over_state_pension_age_at_tax_year_start(), self.were_you_under_16_at_tax_year_start(), self.were_you_not_resident_in_uk_during_the_tax_year(), self.are_you_a_trustee__executor_or_administrator(), self.are_you_a_diver()]
        return uf.all_conditions_are_false(conditions)
    ...
    def did_you_get_dividends_income(self):
        ...
        total = self.transactions.fetch_total_by_tax_year_category_like(tax_year, category_like)
        return total > 0
    ...
    def did_you_get_income_from_property_let_jointly(self):
        how_many_properties_do_you_rent_out = self.get_how_many_properties_do_you_rent_out()
    ...
    def format_breakdown(self, breakdown) -> str:
        ...
        formatted_lines = [' | '.join((field.strip().ljust(width) if index != 4 else field.strip().rjust(width) for (index, (field, width)) in enumerate(zip(line, max_widths)))) for line in fields]
    ...
    def get_business_income(self):
        return self.get_trading_income() + self.get_other_business_income_not_included_as_trading_income()
    ...
    def get_class_4_nics_due(self):
        ...
        if trading_profit <= lower_profits_limit:
            ...
            class_4_nics_due = (trading_profit - lower_profits_limit) * class_4_nics_lower_rate
        else:
            lower_band_nic = (upper_profits_limit - lower_profits_limit) * class_4_nics_lower_rate
            upper_band_nic = (trading_profit - upper_profits_limit) * class_4_nics_upper_rate
    ...
    def get_combined_tax_digest(self):
        ...
        (tax, unused_allowance) = self.calculate_tax(combined_taxable_profit, personal_allowance)
    ...
    def get_cost_to_replace_residential_domestic_items(self) -> float:
        ...
        cost_to_replace_residential_domestic_items = self.get_total_transactions_by_category_like(category_like)
        self.l.debug(f'cost_to_replace_residential_domestic_items: {cost_to_replace_residential_domestic_items}')
    ...
    def get_digest_type_categories(self):
        person_code = self.person_code
        return {'savings': ' INT ', 'dividends': ' DIV ', 'trading': ' SES ', 'property': ' UKP '}
    ...
    def get_dividends_from_uk_companies(self):
        person_code = self.person_code
        tax_year = self.tax_year
        return self.get_year_category_total(tax_year, f'HMRC {person_code} INC Dividends from UK companies')

    def get_dividends_income(self):
        person_code = self.person_code
        tax_year = self.tax_year
        category_like = f'HMRC {person_code} DIV income: '
        dividends_income = self.transactions.fetch_total_by_tax_year_category_like(tax_year, category_like)
    ...
    def get_foreign_dividends(self):
        person_code = self.person_code
        tax_year = self.tax_year
        return self.get_year_category_total(tax_year, f'HMRC {person_code} INC Foreign dividends')
    ...
    def get_hmrc_allowance(self):
        ...
        hmrc_allowance = personal_allowance + allowance_enlargement - allowance_reduction
    ...
    def get_hmrc_businesses(self):
        ...
        query = self.transactions.query_builder().select_raw('DISTINCT Category').where(where).build()
        rows = self.sql.fetch_all(query)
    ...
    def get_hmrc_calculation(self) -> str:
        ...
        def add_part_personal_allowance():
            personal_allowance = self.get_personal_allowance()
            add_hmrc_part('Personal Allowance', personal_allowance)

        def add_part_property_profit():
            property_profit = self.get_property_profit()
            if property_profit > 0:
                add_hmrc_part('Profit from UK land and property', property_profit)
        ...
        def add_part_total_income_received():
        ...
        def add_part_uk_interest():
            savings_income = self.get_savings_income()
            self.l.debug(f'savings_income: {savings_income}')
            if savings_income > 0:
                add_hmrc_part('Interest from UK banks, building societies and securities etc', savings_income)

        def get_revised_basic_rate_limit(pension_payments) -> float:
            basic_rate_limit = get_basic_rate_limit()
            revised_basic_rate_limit = basic_rate_limit + pension_payments
            return revised_basic_rate_limit
        hmrc_parts = ['']
        add_part_self_employment_profit()
        add_part_property_profit()
        add_part_uk_interest()
        add_part_dividends()
        add_part_total_income_received()
        add_part_minus()
        add_part_personal_allowance()
        add_part_marriage_allowance()
        add_part_total_income()
        trading_profit = self.get_trading_profit()
        property_profit = self.get_property_profit()
        personal_allowance = self.get_personal_allowance()
        savings_income = self.get_savings_income()
        ...
        revised_basic_rate_limit = get_revised_basic_rate_limit(pension_payments)
        combined_taxable_profit = self.get_combined_taxable_profit()
        personal_allowance = self.get_personal_allowance()
        ...
        if pension_payments > 0:
            ...
            add_hmrc_part(part, revised_basic_rate_limit)
        ...
        if p_taxable_amount > 0:
            hmrc_parts.append('Pay, pensions, profit etc.')
            taxable_amount_gbp = self.gbp(p_taxable_amount)
        ...
        if s_taxable_amount > 0:
            hmrc_parts.append('Savings interest from banks or building societies, securities etc.')
            savings_basic_rate = self.get_savings_basic_rate()
            savings_nil_band = self.get_savings_nil_band()
            savings_income = self.get_savings_income()
            ...
            add_hmrc_part(label, nil_rate_tax)
        ...
        add_hmrc_part('Total Class 2 National Insurance contributions due', class_2_nics)
        income_tax = self.get_income_tax()
        class_2_nics = self.get_class_2_nics_due()
        total_for_this_year = income_tax + class_2_nics
        add_hmrc_part('Total tax + NICs due for this year', total_for_this_year)
        return '\n'.join(hmrc_parts)
    ...
    def get_hmrc_total_income_received(self) -> float:
        values = [self.get_trading_profit(), self.get_property_profit(), self.get_savings_income(), self.get_dividends_income()]
    ...
    def get_how_many_employments(self):
        person_code = self.person.code
        tax_year = self.tax_year
        query = self.transactions.query_builder().select_raw('COUNT(DISTINCT Category)').where(f'"Tax year"="{tax_year}" AND "Category" LIKE "HMRC {person_code} EMP%"').build()
        how_many = self.sql.fetch_one_value(query)
        return how_many
    ...
    def get_how_many_properties_do_you_rent_out(self):
        person_code = self.person.code
        tax_year = self.tax_year
        query = self.transactions.query_builder().select_raw('COUNT(DISTINCT Category)').where(f'"Tax year"="{tax_year}" AND "Category" LIKE "HMRC {person_code} UKP income%"').build()
        how_many = self.sql.fetch_one_value(query)
        return how_many
    ...
    def get_how_many_self_employed_businesses_did_you_have(self):
        person_code = self.person.code
        tax_year = self.tax_year
        query = self.transactions.query_builder().select_raw('COUNT(DISTINCT Category)').where(f'"Tax year"="{tax_year}" AND "Category" LIKE "HMRC {person_code} SES income%"').build()
        how_many = self.sql.fetch_one_value(query)
        return how_many
    ...
    def get_income_tax(self):
        ...
        self.l.debug(f'dividends_income: {dividends_income}')
        tax_free_allowance = self.get_hmrc_allowance()
        (non_savings_tax, available_allowance) = self.calculate_tax(non_savings_income, tax_free_allowance)
        self.l.debug(f'non_savings_tax: {non_savings_tax}')
        self.l.debug(f'available_allowance: {available_allowance}')
        (savings_tax, available_allowance) = self.calculate_savings_tax(savings_income, available_allowance)
        self.l.debug(f'savings_tax: {savings_tax}')
        self.l.debug(f'available_allowance: {available_allowance}')
        (dividends_tax, available_allowance) = self.calculate_dividends_tax(dividends_income, available_allowance)
    ...
    def get_legal__management_and_other_professional_fees(self) -> float:
        ...
        legal__management_and_other_professional_fees = self.get_total_transactions_by_category_like(category_like)
    ...
    def get_marriage_allowance_transfer_amount(self) -> float:
        ...
        if total_income_excluding_tax_free_savings > personal_allowance:
            return 0
        return min(marriage_allowance, personal_allowance - total_income_excluding_tax_free_savings)

    def get_marriage_allowance_transfer_amount_gbp(self) -> str:
        return self.gbpb(self.get_marriage_allowance_transfer_amount())
    ...
    def get_marriage_allowance_transferred_amount(self) -> float:
        ...
        marriage_allowance_transferred_amount = spouse_hmrc.get_marriage_allowance_transfer_amount()
        spouse_hmrc.enable_debug()
        self.l.debug('Enabled debug for spouse HMRC instance')
        self.l.debug(f'marriage_allowance_transferred_amount: {marriage_allowance_transferred_amount}')
    ...
    def get_net_business_loss_for_tax_purposes(self):
        ...
        net_business_loss_for_tax_purposes = min(0, income - self.get_trading_expenses_actual()) * -1
    ...
    def get_net_business_profit_for_tax_purposes(self):
        ...
        if self.use_trading_allowance():
            net_business_profit_for_tax_purposes = max(0, income - self.get_trading_allowance_actual())
        else:
            net_business_profit_for_tax_purposes = max(0, income - self.get_trading_expenses_actual())
    ...
    def get_other_business_income_not_trading_income_gbp(self):
        return self.gbpb(self.get_other_business_income_not_included_as_trading_income())
    ...
    def get_other_dividends(self):
        person_code = self.person_code
        tax_year = self.tax_year
        return self.get_year_category_total(tax_year, f'HMRC {person_code} INC Other dividends')
    ...
    def get_overview_parts(self) -> list:
        ...
        steps = ['get_trading_digest', 'get_property_digest', 'get_savings_digest', 'get_dividends_digest', 'get_final_digest', 'get_combined_tax_digest', 'get_savings_tax_digest', 'get_dividends_tax_digest', 'get_marriage_allowance_digest']
        parts = []
        for step in steps:
    ...
    def get_payments_to_pension_schemes__relief_at_source(self):
        person_code = self.person_code
        payments_to_pension_schemes = self.transactions.fetch_total_by_tax_year_category_like(self.tax_year, f'HMRC {person_code} RLF pension')
    ...
    def get_pension_contributions(self):
        person_code = self.person_code
        pension_contributions = self.transactions.fetch_total_by_tax_year_category(self.tax_year, f'HMRC {person_code} RLF pension contribution')
    ...
    def get_pensions(self):
        ...
        return self.transactions.fetch_total_by_tax_year_category_like(tax_year, category_like)
    ...
    def get_property_expenses_actual(self) -> float:
        property_expenses = [self.get_rent__rates__insurance_and_ground_rents(), self.get_property_repairs_and_maintenance(), self.get_legal__management_and_other_professional_fees()]
    ...
    def get_property_income(self) -> float:
        ...
        property_income = self.transactions.fetch_total_by_tax_year_category_like(tax_year, category_like)
    ...
    def get_property_repairs_and_maintenance(self) -> float:
        ...
        repairs_and_maintenance = self.get_total_transactions_by_category_like(category_like)
    ...
    def get_rented_property_postcode(self):
        person_code = self.person.code
        tax_year = self.tax_year
        query = self.transactions.query_builder().select_raw('DISTINCT Category').where(f'"Tax year"="{tax_year}"' + f' AND "Category" LIKE "HMRC {person_code} UKP income: rent received %"').build()
        self.l.debug(f'query: {query}')
    ...
    def get_savings_income(self) -> float:
        ...
        savings_income = self.transactions.fetch_total_by_tax_year_category_like(tax_year, category_like)
    ...
    def get_savings_tax_digest(self):
        ...
        parts = [f'taxable savings: {taxable_savings_gbp}', f'unused personal allowance: {unused_allowance_gbp}', f'taxable amount: {taxable_amount_gbp}', f'tax: {tax_gbp}']
        return '\n' + ' | '.join(parts)
    ...
    def get_taxed_uk_interest(self):
        person_code = self.person_code
        tax_year = self.tax_year
        return self.get_year_category_total(tax_year, f'HMRC {person_code} INT income: interest UK taxed')
    ...
    def get_total_of_any__one_off__payments_in_box_1(self):
        ...
        hmrc_contribution = self.transactions.fetch_total_by_tax_year_category(self.tax_year, 'HMRC S pension tax relief')
    ...
    def get_total_taxable_income(self):
        ...
        values = [self.get_trading_profit(), self.get_property_profit(), self.get_taxable_savings(), self.get_taxable_dividends()]
    ...
    def get_total_taxable_profits_from_this_business(self):
        net_business_profit_for_tax_purposes = self.get_net_business_profit_for_tax_purposes()
        other_business_income_not_already_included = self.get_other_business_income_not_already_included()
        loss_brought_forward_set_off_against_profits = self.get_trading_loss_brought_forward_set_off_against_profits()
        total_taxable_profits_from_this_business = net_business_profit_for_tax_purposes + other_business_income_not_already_included - loss_brought_forward_set_off_against_profits
    ...
    def get_total_to_add_to_sa_account_due_by_31st_january(self):
        values = [self.get_total_tax_due(), self.get_first_payment_on_account_for_next_year()]
    ...
    def get_trading_expenses_actual(self):
        ...
        trading_expenses = self.transactions.fetch_total_by_tax_year_category_like(tax_year, category_like)
    ...
    def get_trading_income(self) -> float:
        ...
        tax_year = self.tax_year
        category_like = f'HMRC {person_code} SES income'
        trading_income = self.transactions.fetch_total_by_tax_year_category_like(tax_year, category_like)
    ...
    def get_trading_loss(self):
        trading_loss = max(0, self.get_trading_expenses_actual() - self.get_trading_income())
        return trading_loss
    ...
    def get_trading_loss_set_off_against_other_income_this_tax_year_gbp(self):
        return self.gbpb(self.get_trading_loss_set_off_against_other_income_this_tax_year())
    ...
    def get_untaxed_foreign_interest(self):
        person_code = self.person_code
        tax_year = self.tax_year
        return self.get_year_category_total(tax_year, f'HMRC {person_code} INT income: interest foreign untaxed')
    ...
    def get_untaxed_uk_interest(self):
        person_code = self.person_code
        tax_year = self.tax_year
        untaxed_uk_interest = self.get_year_category_total(tax_year, f'HMRC {person_code} INT income: interest UK untaxed')
    ...
    def list_categories(self):
        query = self.transactions.query_builder().select_raw('DISTINCT Category').where(f'"Tax year" = "{self.tax_year}" AND "Category" LIKE "HMRC {self.person_code}%" ORDER BY Category').build()
    ...
    def print_reports(self):
        for report_type in HMRC_Output.REPORT_TYPES:
            ...
            output_details = {'answers': self.get_answers(), 'person_name': self.get_person_name(), 'report_type': report_type, 'tax_year': self.tax_year, 'unique_tax_reference': self.get_unique_tax_reference()}
    ...
    def were_you_employed_in_this_tax_year(self) -> bool:
        person_code = self.person.code
        tax_year = self.tax_year
        query = self.transactions.query_builder().select_raw('COUNT(*)').where(f'"Tax year"="{tax_year}" AND "Category" LIKE "HMRC {person_code} EMP%income"').build()
        how_many = self.sql.fetch_one_value(query)
        return how_many > 0
