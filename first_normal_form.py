        ...
        for table in tables:
            ...
            if rows:
                for col_idx, col in enumerate(columns):
                    col_name = col[1]
                    check_composite_values(table_name, col_name, [row[col_idx] for row in rows], violations)
                    check_data_type_consistency(table_name, col_name, [row[col_idx] for row in rows], violations)
...
def check_repeating_columns(table_name: str, column_names: List[str], violations: Dict) -> None:
    """
...
def check_composite_values(table_name: str, column_name: str, values: List[Any], violations: Dict) -> None:
    """
...
def check_data_type_consistency(table_name: str, column_name: str, values: List[Any], violations: Dict) -> None:
    """
